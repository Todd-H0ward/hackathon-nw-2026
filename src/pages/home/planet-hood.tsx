...
export const PlanetHood = ({ activeBody, className }: PlanetHoodProps) => {
  const info = getPlanetInfo(activeBody);
    ...
    >
      <div className="bg-linear-to-t from-[#2a2b2e] from-35% via-[#2a2b2e]/92 via-70% to-transparent px-4 pt-20 pb-[max(1.5rem,env(safe-area-inset-bottom))] md:px-8">
        <div className="mx-auto flex max-w-3xl flex-col gap-4">
          ...
          <dl
            key={`${info.id}-stats`}
            className="grid grid-cols-3 gap-x-3 gap-y-3 sm:grid-cols-6"
          >
            {info.stats.map((stat) => (
